...
void find_test() {
    ...
    auto location = find(begin(vec), end(vec), 1);
    ...
    list<Person> players{{"Larry", 18}, {"Moe", 20}, {"Curly", 21}};

    auto location1 = find(players.begin(), players.end(), Person{"Moe", 20});
    if (location1 != players.end()) {
        cout << "Found Moe" << endl;
    } else {
